import SparkleText from "../animation/Sparkles";

function AboutSection() {
  return (
    <section className="about-section flex flex-col text-justify gap-y-4 ">
        
      <p className="">
        I'm a whimsy software and ai engineer, curious about the digital world, with a dream of one day being affectionately referred to as a "nerd🤓" (at least for the next 5 years, before I pivot my obessession into something else). I am currently developing AI solutions at a large FinTech consulting firm, <span className="font-semibold"><a href="https://synechron.com" className="underline hover:text-rose-500 transition-colors duration-200" target="_blank" rel="noopener noreferrer">Synechron</a></span>, building ML pipelines for major financial institutions.
        </p>

      <p className="">
        I majored in <span className="font-semibold">Electrical and Electronics Engineering</span> at <span className="font-semibold"><a href="https://www.ntu.edu.sg/" target="_blank" rel="noopener noreferrer" className="underline hover:text-rose-500 transition-colors duration-200">Nanyang Technological University, Singapore (NTU)</a></span>, which strangely sparked an interest for computer science? I'm really passionate for projects that explores the intersection of engineering and design, and also ones that involves solving challenging but meaningful problems: improving accessibility; or simply for the sake of fun and enjoyment.
      </p>

      <p className="">
        I like to unwind by playing video games with my friendos (Overwatch!!!), playing the guitar, learning a new musical instrument, recording singing covers of pop songs, or hanging with my besties. 
      </p>

      <p>
        I always strive to do my best and learn as much as I can in this short life I have, and am always looking for opportunities to learn, grow and contribute my skills effectively. Most importantly, <span className="font-semibold">I want to put as much cool stuff into the world as I can</span>, and I’m excited to see where this journey takes me! :D
      </p>


      <div className="flex font-semibold">
        <SparkleText>Slay vibes only!!! 🎮🎸🎤👯🩵</SparkleText>
      </div>

    </section>
  );
}

export default AboutSection;